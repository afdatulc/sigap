...
<div class="space-y-6 max-w-4xl mx-auto">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Pendaftaran Usaha Baru</h2>
            <p class="text-sm text-gray-500 mt-1">Masukkan informasi bisnis lokal dan tandai lokasi usahanya di peta.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.usaha.store') }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="nama_usaha" value="{{ old('nama_usaha') }}" class="w-full py-2 px-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 transition-colors" placeholder="Contoh: Warung Barokah" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Pemilik</label>
                        <input type="text" name="pemilik_usaha" value="{{ old('pemilik_usaha') }}" class="w-full py-2 px-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 transition-colors" placeholder="Nama lengkap pemilik" required>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="nomor_telepon" value="{{ old('nomor_telepon') }}" class="w-full py-2 px-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 transition-colors" placeholder="081234567890">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi Singkat Usaha</label>
                        <textarea name="deskripsi_usaha" rows="2" class="w-full py-2 px-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 transition-colors" placeholder="Penjelasan mengenai barang/jasa yang ditawarkan">{{ old('deskripsi_usaha') }}</textarea>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Alamat Tempat Usaha</label>
                        <textarea name="alamat_usaha" rows="2" class="w-full py-2 px-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 transition-colors" placeholder="Jalan, RT/RW, Dusun" required>{{ old('alamat_usaha') }}</textarea>
                    </div>
                    <div class="md:col-span-2">
                        <label class="flex items-center gap-3 p-4 bg-gray-50 border border-gray-200 rounded-lg cursor-pointer">
                            <input type="checkbox" name="status_aktif" value="1" {{ old('status_aktif', true) ? 'checked' : '' }} class="w-5 h-5 text-emerald-600 rounded border-gray-300 focus:ring-emerald-500">
                            <div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Standard marker, but we will stylize it with CSS filter to look blueish
