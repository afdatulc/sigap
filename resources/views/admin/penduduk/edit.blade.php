...
<div class="space-y-6 max-w-4xl mx-auto">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Data Penduduk</h2>
            <p class="text-sm text-gray-500 mt-1">Perbarui data demografi dan lokasi rumah untuk {{ $penduduk->nama_lengkap }}.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.penduduk.update', $penduduk->id) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="nik" value="{{ old('nik', $penduduk->nik) }}" class="w-full py-2 px-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 transition-colors" placeholder="16 Digit NIK" required maxlength="16">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Lengkap</label>
                        <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $penduduk->nama_lengkap) }}" class="w-full py-2 px-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 transition-colors" placeholder="Sesuai KTP" required>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Alamat Lengkap</label>
                        <textarea name="alamat" rows="2" class="w-full py-2 px-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 transition-colors" placeholder="Nama jalan, RT/RW, Dusun">{{ old('alamat', $penduduk->alamat) }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="status_listrik" class="w-full py-2 px-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 transition-colors" required>
                            ...
                            <option value="Memiliki" {{ old('status_listrik', $penduduk->status_listrik) == 'Memiliki' ? 'selected' : '' }}>Memiliki Meteran Listrik</option>
                            <option value="Belum Memiliki" {{ old('status_listrik', $penduduk->status_listrik) == 'Belum Memiliki' ? 'selected' : '' }}>Belum Memiliki Listrik</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="status_ekonomi" class="w-full py-2 px-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 transition-colors" required>
                            <option value="">-- Pilih Status --</option>
                            <option value="Mampu" {{ old('status_ekonomi', $penduduk->status_ekonomi) == 'Mampu' ? 'selected' : '' }}>Mampu (Berkecukupan)</option>
                            <option value="Menengah" {{ old('status_ekonomi', $penduduk->status_ekonomi) == 'Menengah' ? 'selected' : '' }}>Menengah</option>
                            <option value="Kurang Mampu" {{ old('status_ekonomi', $penduduk->status_ekonomi) == 'Kurang Mampu' ? 'selected' : '' }}>Kurang Mampu (Miskin)</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" id="latitude" name="latitude" value="{{ old('latitude', $penduduk->latitude ?? '-6.200000') }}" class="w-full py-2 px-3 bg-gray-100 border border-gray-300 rounded-lg text-gray-600 cursor-not-allowed" readonly>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Longitude</label>
                        <input type="text" id="longitude" name="longitude" value="{{ old('longitude', $penduduk->longitude ?? '106.816666') }}" class="w-full py-2 px-3 bg-gray-100 border border-gray-300 rounded-lg text-gray-600 cursor-not-allowed" readonly>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var map = L.map('map').setView([initialLat, initialLng], {{ $penduduk->latitude ? 16 : 13 }});
